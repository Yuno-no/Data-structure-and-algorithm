...
public class shortestPath {
    ...
    public int[] getShortestPaths(int[][] adjMatrix) {
        ...
        path = new int[adjMatrix.length];  //用于存放最短路径上次经过的定点
        used[0] = true;  //表示顶点0已被遍历
        for(int i = 1;i < adjMatrix.length;i++) {
            result[i] = adjMatrix[0][i];
            used[i] = false;
            path[i] = (adjMatrix[0][i] != -1) ? 0 : -1;
        }
        ...
        for(int i = 1;i < adjMatrix.length;i++) {
            ...
            for(int j = 1;j < adjMatrix.length;j++) {
                ...
                if(!used[j]) {  //当顶点j未被遍历时
                    //首先，顶点k到顶点j要能通行；
                    // 这时，当顶点0到顶点j的距离大于顶点0到k再到j的距离或者顶点0无法直接到达顶点j时，更新顶点0到顶点j的最短距离
    ...
    public static void main(String[] args) {
        ...
        System.out.println();
        System.out.println("顶点0到图中所有顶点最短距离上一次经过的顶点为：");
